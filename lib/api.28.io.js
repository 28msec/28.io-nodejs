exports.$28 = function(API_URL){
    ...
    var PROJECT_DOMAIN = API_URL.substring(API_URL.indexOf('//') + '//'.length).split('/')[0];
    if(!(/^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(:[0-9]+)?$/.test(PROJECT_DOMAIN))) {
        PROJECT_DOMAIN = PROJECT_DOMAIN.split('.').slice(1).join('.');
    } else {
        isIP = true;
    }
